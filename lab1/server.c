...
void *new_client(void *arg) {
    ...
    while(1) {
        ...
        // User logged in, join session
            ...
            char* sessionId = calloc(1, 20*sizeof(char));
            strcpy(sessionId, (char *)(pktRecv.data));
            printf("User %s: Trying to join session %s...\n", newUsr -> uname, sessionId);

            // Fails if session not exists
            if(isValidSession(sessionList, sessionId) == NULL) {
                pktSend.type = JN_NAK;
                toSend = 1;
                int cursor = sprintf((char *)(pktSend.data), "%s", sessionId);
            ...
            // Success join session
                ...
                pktSend.type = JN_ACK;
                toSend = 1;
                sprintf((char *)(pktSend.data), "%s", sessionId);
        ...
        // User logged in, leave session, no reply
            ...
            while(sessJoined != NULL) {
                char* curSessId = sessJoined -> sessionId;
        ...
        // User create new session 
            ...
            packetToString(&pktSend, buffer);
            pthread_mutex_lock(&sessionList_mutex);
            sessionList = init_session(sessionList, sessionId);
            printf("fffffffffffdcreate %s",sessionList -> sessionId);
            pthread_mutex_unlock(&sessionList_mutex);
            ...
            pthread_mutex_lock(&sessionList_mutex);
            sessionList = join_session(sessionList, sessionId, newUsr);
            printf("fffffffffffdfind %s",sessionList -> sessionId);
            pthread_mutex_unlock(&sessionList_mutex);
            ...
            for(User *usr = userLoggedin; usr != NULL; usr = usr -> next) {
                if(strcmp(usr -> uname, source) == 0) {
                    usr -> inSession = 1;
                    usr -> sessJoined = init_session(usr -> sessJoined, sessionId);
                    printf("createeeeeeeeeqe, %s", userLoggedin -> sessJoined->sessionId);
                }

            }
            pthread_mutex_unlock(&userLoggedin_mutex);
            printf("22222222222222, %s", userLoggedin -> sessJoined->sessionId);
        ...
        // Respond user query
            ...
            // }
            // printf("\n");
            printf("33333333333, %s", userLoggedin -> sessJoined->sessionId);

            for(User *usr = userLoggedin; usr != NULL; usr = usr -> next) {
                cursor += sprintf((char *)(pktSend.data) + cursor, "%s", usr -> uname);
                for(Session *sess = usr -> sessJoined; sess != NULL; sess = sess -> next) {
                    cursor += sprintf((char *)(pktSend.data) + cursor, "\t%s", sess -> sessionId);
                }
                pktSend.data[cursor++] = '\n';
            }
