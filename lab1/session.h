 ...
 */
Session *isValidSession(Session *sessionList, char* sessionId) {
    Session *current = sessionList;
    while(current != NULL) {
        if(strcmp(current -> sessionId , sessionId) == 0) {
            printf("fffffffffffound %s",current -> sessionId);
            return current;
        }
...
bool inSession(Session *sessionList, char* sessionId, const User *usr) {
 ...
 */
Session *init_session(Session *sessionList, char* sessionId) {
    Session *newSession = calloc(sizeof(Session), 1);
    newSession -> sessionId = sessionId;
    printf("fffffffffffcreate %s",newSession -> sessionId);
...
Session *join_session(Session *sessionList, char* sessionId, const User *usr) {
 ...
 */
Session* remove_session(Session* sessionList, char* sessionId) {
    // Search for this session from sessionList
    assert(sessionList != NULL);

    // First in list
    if(strcmp(sessionList -> sessionId , sessionId) == 0) {
        ...
        Session *prev;
        while(cur != NULL) {
            if(strcmp(cur -> sessionId, sessionId) == 0) {
                prev -> next = cur -> next;
                free(cur);
                break;
            } else {
...
Session *leave_session(Session *sessionList, char* sessionId, const User *usr) {
    // Check session exists outside & Find current session
    Session *cur = isValidSession(sessionList, sessionId);
    assert(cur != NULL);
